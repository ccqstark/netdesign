...
public class RmiClientFX extends Application {
    ...
    private final Button btnLogin = new Button("登录");
    //学号和姓名的文本框
    private final TextField tfNO = new TextField();
    ...
    private void initComponents(Stage primaryStage) {
        ...
        hBoxInput.setPadding(new Insets(2, 20, 10, 20));
        tfMsg.setPrefWidth(500);
        hBoxInput.getChildren().addAll(tfMsg, btnSend, btnExit);

        mainPane.setTop(hBoxLogin);
        mainPane.setCenter(vBox);
        mainPane.setBottom(hBoxInput);
    ...
    private void initRmi() {
        try {
            ...
            String ip = "10.173.216.190";
            // String ip = "10.173.216.120";
    ...
    private void initEvent(Stage primaryStage) {
        ...
        btnLogin.setOnAction(event ->
        {
            try {
                ...
                //服务端如果重启，就会失去rmi连接，引发此异常
                // 捕获此异常重新初始化rmi连接，并再次登录
                new Thread(() -> {
                    initRmi();
                    btnLogin.fire();
                }).start();
        ...
        tfMsg.setOnKeyPressed((KeyEvent keyEvent) -> {
